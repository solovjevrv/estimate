...
Два клиентских поля помогают отличить намеренное действие от повторного:

- `start_new_round` принимает `fromRoundId` — раунд, который клиент видел текущим. Если стол уже
  ушёл вперёд, сервер вернёт актуальный раунд вместо нового: двойной клик и два скрам-мастера не
  наплодят пустых раундов.
- `update_links` принимает `version` — версию ссылок из последнего состояния. Если за это время
  ссылки правил кто-то другой, приходит `conflict`, а не молчаливая перезапись чужого текста.
  Без `version` поведение прежнее: побеждает последний.
