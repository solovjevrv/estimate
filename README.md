...
- `submit_vote` принимает `roundId` — задачу, за которую голосуют. Оценка, отправленная в момент
  смены раунда, не попадёт в следующую задачу: участник получит `conflict`.
- `update_links` принимает `roundId` и `version` — раунд и версию ссылок из последнего состояния.
  ...
  чужого текста. Без этих полей поведение прежнее: побеждает последний.

У комнаты есть `revision` — номер изменения стола, растущий с каждым действием. Рассылки идут
параллельно и могут обогнать друг друга, поэтому клиенту стоит отбрасывать снимки с номером
меньше уже показанного.
