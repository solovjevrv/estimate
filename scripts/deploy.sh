...
echo "== Собираю образы (последовательно: сервер с 1 ядром, экономим CPU) =="
...
if ! curl -fsS --resolve estimate.solovyovdev.ru:443:127.0.0.1 \
  https://estimate.solovyovdev.ru/health >/dev/null; then
