 ...
 *
 * Редактор открывается двойным кликом по SVG-пути стрелки (см. комментарий в
 * `BoardFloatingEdge.vue`): для пустой связи HTML-оверлей подписи не рендерится,
 * поэтому клик попадает на путь и Vue Flow эмиттит `@edge-double-click`. Чтобы путь
