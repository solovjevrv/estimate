  ...
  app.post<{ Params: { id: string } }>(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        // assertViewAccess проверяет view, но для upload нужно edit
        // Повторная проверка с edit:
        const board = await boardsService['requireBoard'](boardId);
        await boardsService['assertAccess'](board, req.user.sub, 'edit', boardsService['teams']);
      } catch (err) {
  ...
  app.get<{ Params: { id: string; filename: string } }>(
    '/api/boards/:id/assets/:filename',
    {
      schema: {
        ...
        description: 'Требует view-доступ к доске (членство в команде или владение личной доской).',
        params: {
          type: 'object',
...
export const boardImagesPlugin = fp(boardImagesPluginImpl, {
  ...
  dependencies: ['poker-auth', 'poker-boards'],
});
