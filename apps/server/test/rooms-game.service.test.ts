/**
 * Юнит-тесты правил стола (голосование, раунды, вход, ссылки) без БД:
 * репозитории подменены заглушками. CRUD и жизненный цикл самой комнаты —
 * в rooms-service.test.ts (16.5).
 */
...
function serviceWith(
  ...
  return new RoomsGameService(
    ...
    new RoomsTransactions(db, new TeamAccess(teams as TeamsRepository)),
  );
}
...
describe('RoomsGameService: голосование', () => {
...
describe('RoomsGameService: права на управление раундом', () => {
...
describe('RoomsGameService: вход за стол', () => {
...
describe('RoomsGameService: ссылки на задачу', () => {
...
describe('RoomsGameService: архивная комната блокирует действия за столом', () => {
 ...
 * конструкторе RoomsTransactions (7.30) позволяет подменить репозиторий
 * заглушкой напрямую, без глобального vi.spyOn(RoomsRepository.prototype, ...)
 * и без БД.
 */
describe('RoomsGameService.getState (7.30)', () => {
  it('строит снимок стола из данных репозитория без обращения к БД', async () => {
    ...
    const service = new RoomsGameService(
      new RoomsRepository(db),
      {} as UsersRepository,
      new GuestSessions(GUEST_SECRET, 'guest'),
      new RoomsTransactions(db, TeamAccess.forExecutor(db), () => fakeRepo as RoomsRepository),
    );
