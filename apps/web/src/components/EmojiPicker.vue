 ...
 *
 * Проп по умолчанию выключен (`false`) — включается точечно там, где пикер
 * используется для обзора каталога, а не для одной быстрой реакции: вставка
 * эмодзи-элемента на доску, «Заменить эмодзи» в тулбаре выделения, реакция на
 * стикер (13.х). Быстрая реакция на карточке участника в комнате (10.10) —
 * единственное исключение, специально БЕЗ свёртки: решение пользователя
 * 27.08.2026 после регрессии живым тестом (клик по 👍, который не в первой
 * категории каталога, а тест ожидал мгновенный доступ без разворота).
 */
