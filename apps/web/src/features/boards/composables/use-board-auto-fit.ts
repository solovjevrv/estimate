 ...
 * `:max-zoom` `<VueFlow>` (сейчас 2000%, было 200% до 22.2): для доски с одним
 * маленьким стикером итог — зум сразу на весь потолок после создания первого
 * элемента (воспроизведено в `boards-frames-groups.spec.ts`). Автофит без
 * явного действия пользователя не должен приближать БЛИЖЕ 100% — обычный
 * ручной fit-view (Ctrl+1, тулбар) по-прежнему волен использовать весь
 * `:max-zoom`.
 *
