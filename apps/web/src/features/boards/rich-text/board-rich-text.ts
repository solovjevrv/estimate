 ...
 * зависят от `editableEl` и не должны мешать обычному коммиту: клик «Дублировать»
 * посреди набора текста переставал сохранять черновик (коммит просто не
 * срабатывал), а глобальные хоткеи переставали работать после любого клика по
 * тулбару, пока фокус не уходил ещё куда-то. Найдено ревью, не живой проверкой —
 * сценарий («выделить, начать печатать, кликнуть Дублировать не блюрясь явно»)
 * не был в ручных тестах.
 *
 ...
 *
 * `activeMarks` — метки текущей ЦЕЛИ форматирования, а не только реального
 * DOM-выделения: при схлопнутом курсоре без выделения и непустом тексте они
 * вычисляются по всему тексту, чтобы начертание/маркер применялись к нему.
 * `hasTextSelection` различает эти два случая — ссылка требует ЯВНОГО
 * выделения и не использует fallback «весь текст».
 */
export interface BoardTextEditorHandle {
  itemId: string;

  /**
   * Метки текущей цели форматирования:
   * - непустое выделение — метки выбранного фрагмента;
   * - нет выделения, но текст непустой — метки всего текста;
   * - текста нет — null.
   */
  activeMarks: Ref<BoardTextMark | null>;

  /** Только фактическое непустое DOM-выделение; необходимо для ссылок. */
  hasTextSelection: Ref<boolean>;
