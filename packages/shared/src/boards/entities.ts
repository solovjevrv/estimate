...
/** Максимальное ручное смещение подписи связи (12.18) — мировые px канваса, защита от мусорных значений */
...
export interface BoardEdgeStyle {
  ...
  /**
   * Ручное смещение подписи связи (12.18) от базовой точки подписи
   * (геометрическая середина пути либо апекс кривой при заданном
   * curveOffset — та же точка, что уже вычисляется как labelX/labelY в
   * BoardFloatingEdge.vue) — в мировых px канваса. Смещение ОТНОСИТЕЛЬНОЕ:
   * если базовая точка сама сдвигается, подпись едет вместе с ней, сохраняя
   * то же смещение, а не абсолютные мировые координаты.
