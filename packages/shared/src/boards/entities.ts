 ...
 * других элементов. Вынесена в `@estimate/shared`, чтобы и сервер, и клиент
...
export interface BoardItemStyle {
  ...
  /**
   * Размер бокса (ширина/высота), для которого `fontSize` актуален (26.08.2026) —
   * не текущая геометрия элемента (`BoardItem.width/height` меняется при КАЖДОМ
   * resize независимо от режима), а геометрия на момент, когда `fontSize`
   * последний раз был установлен верно: явным вводом в `manual`, либо resize'ом
   * в `auto`. В `manual` resize двигает бокс, но НЕ обновляет ни `fontSize`, ни
   * этот якорь — величина расхождения между якорем и текущим боксом накапливается
   * и используется только в момент переключения обратно в `auto`
   * (`setSelectedFontSizeMode` в `use-board-selection.ts`), чтобы досчитать
   * пропущенные resize'ы одним пересчётом, а не молча игнорировать их (баг из
   * живой проверки: `manual=4` → resize 2x → переключение на `auto` не меняло
   * число, хотя бокс уже вырос). Не задано — считать якорем текущий бокс
   * элемента (нет накопленного расхождения, поведение как до этой доработки).
   */
  fontSizeBoxWidth?: number;
  fontSizeBoxHeight?: number;
...
export interface BoardEdge {
  ...
  /**
   * Слой отображения связи (12.21) — общее пространство порядка со `zIndex`
   * карточек (`BoardItem.zIndex`), не отдельное. По умолчанию при создании
   * связь встаёт ВЫШЕ всех текущих карточек и связей (Miro-приём: стрелка
   * всегда видна поверх объектов), карточки при создании продолжают
   * упорядочиваться только относительно друг друга — так связи остаются
   * поверх новых карточек тоже, без явного действия пользователя. Двигается
   * пользователем на передний/задний план через контекстное меню связи, тем
   * же способом, что и у карточек.
   */
  zIndex: number;
}
